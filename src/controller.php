...
if (isset($_POST['user_m'])) {
  ...
  $txt .= "<table><thead><th>User Id</th><th>User Name</th><th>Email</th><th>Contact</th><th>Tagline</th><th>Password</th><th>Date Of Joining</th><th>Role</th><th>Action</th></thead>";
  foreach ($result as $row) {
    if ($row['role'] == 'user') {
      $txt .= '<tr><td>' . $row['user_id'] . '</td><td>' . $row['name'] . '</td><td>' . $row['email'] . '</td><td>' . $row['contact'] . '</td><td>' . $row['tagline'] . '</td><td>' . $row['password'] . '</td><td>' . $row['date_of_joining'] . '</td><td><select id=' . $row["user_id"] . ' class="selectdrop" name="status" onchange="changeSelect(this.id,this.value,0)">
...
if (isset($_POST['blog_m'])) {
  ...
  $txt .= "<table id='myTable'><thead><th>Post Id</th><th>User Id</th><th>Status</th><th>Published At</th><th>Blog Title</th><th>Status By Admin</th><th>Last Edit</th><th>Action</th></thead>";
  foreach ($result as $row) {
    if ($row['status_by_admin'] == "Pending") {
      $txt .= '<tr><td>' . $row['post_id'] . '</td><td>' . $row['user_id'] . '</td><td>' . $row['status'] . '</td><td>' . $row['published_at'] . '</td><td>' . $row['blog_title'] . '</td><td><select id=' . $row["post_id"] . ' class="selectdrop" name="status" onchange="changeSelect(this.id,this.value,1)">
